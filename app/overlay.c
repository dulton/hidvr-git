...
SDK_ENC_VIDEO_OVERLAY_CANVAS_t* OVERLAY_create_canvas(size_t width, size_t height)
{
	return NULL;
}

void OVERLAY_release_canvas(SDK_ENC_VIDEO_OVERLAY_CANVAS_t* canvas)
{
}

ssize_t OVERLAY_put_text(SDK_ENC_VIDEO_OVERLAY_CANVAS_t* canvas,
	int x, int y, OVERLAY_FONT_SIZE_t font_size, const char *text, OVERLAY_TEXT_STYLE_t style)
{
	...
	SDK_ENC_VIDEO_OVERLAY_PIXEL_t fg_color, bg_color, enclosed_color;
	...
	if(is_enclosed){
		for(i = 0; i < text_height - 1 ; ++i){
			for(ii = 0; ii < text_width - 1 ; ++ii){
				SDK_ENC_VIDEO_OVERLAY_PIXEL_t center_color;
				SDK_ENC_VIDEO_OVERLAY_PIXEL_t right_color, bottom_color, bottom_right_color;
				...
				canvas->get_pixel(canvas, x_base + ii + 1, y_base + i + 1, &bottom_right_color); // to the bottom right
				...
				if(canvas->match_pixel(canvas, bg_color, center_color)){
					...
					}else if(canvas->match_pixel(canvas, fg_color, bottom_color)){
						canvas->put_pixel(canvas, x_base + ii, y_base + i, enclosed_color);
					}else if(canvas->match_pixel(canvas, fg_color, bottom_right_color)){
						canvas->put_pixel(canvas, x_base + ii, y_base + i, enclosed_color);
					}
					...
					if(canvas->match_pixel(canvas, bg_color, bottom_color)){
						canvas->put_pixel(canvas, x_base + ii + 1, y_base + i + 1, enclosed_color);
					}
				}
			}
		}
