...
int NTP_sync(const char* server_domain, const char* server_ip, int timeout, char timezone)
{
	int i = 0;
	int ret = 0;
	int  sock;
...
//	int retry_time = timeout;
	...
	// ////////////////////////////////////////////////////
	// default to GMT+8
	tz.tz_minuteswest = -1 * timezone * 60;
	tz.tz_dsttime = 0;
	...
	if((server_domain && strlen(server_domain) != 0) || server_ip){
		ntp_sock_send(sock, server_domain, server_ip, NTP_SERVER_PORT, send_pts);
	}else{
...
//	sleep(1);
	// wait response
	ret = ntp_sock_recv(sock, timeout, &ntp_pack);
