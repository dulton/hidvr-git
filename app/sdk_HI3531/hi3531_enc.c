...
#define SNAPSHOT_JPEG_GRP (127)
#define SNAPSHOT_JPEG_CHN (127)
...
#define __HI_AOUT_HDMI_DEV (5)
#define __HI_AOUT_HDMI_CHN (0)
#define __HI_AIN_CH (3)
...
static int enc_create_h264_stream(const char* name, int vin, int stream, SDK_ENC_H264_STREAM_ATTR_t* stream_attr)
{
	if(stream_attr && vin < HI_VENC_CH_BACKLOG_REF && stream < HI_VENC_STREAM_BACKLOG_REF){
		...
		if(0 == this_stream_attr->magic){
			...
			int const vpss_ch = (0 == stream) ? VPSS_PRE1_CHN : VPSS_LSTR_CHN;
...
#define MMZ_ZONE_NAME1 "ddr1"
			...
			stMppChnGRP.s32DevId = venc_group;
			stMppChnGRP.s32ChnId = 0;
//			SOC_CHECK(HI_SYS_SetMemConf(HI_ID_GROUP, venc_group, 0, venc_group%2 ?MMZ_ZONE_NAME0 :MMZ_ZONE_NAME1));
			SOC_CHECK(HI_MPI_SYS_SetMemConf(&stMppChnGRP, venc_group%2 ?MMZ_ZONE_NAME0 :((venc_group < 12) ?MMZ_ZONE_NAME0: MMZ_ZONE_NAME1)));

			stMppChnGRP.enModId  = HI_ID_VENC;
			stMppChnGRP.s32DevId = 0;
			stMppChnGRP.s32ChnId = venc_ch;
//			SOC_CHECK(HI_SYS_SetMemConf(HI_ID_VENC,  0, venc_ch, venc_group%2 ?MMZ_ZONE_NAME0 :MMZ_ZONE_NAME1));
			SOC_CHECK(HI_MPI_SYS_SetMemConf(&stMppChnGRP, venc_group%2 ?MMZ_ZONE_NAME0 :((venc_group < 12) ?MMZ_ZONE_NAME0: MMZ_ZONE_NAME1)));
			...
			switch(stream_attr->mode){
				...
				case SDK_ENC_H264_MODE_AUTO:
				{
					...
					p_h264_vbr->u32MaxBitRate = stream_attr->bps;
				...
				case SDK_ENC_H264_MODE_ABR:
				{
					...
					p_h264_cbr->u32BitRate = stream_attr->bps;
			...
			switch(stream_attr->mode) {
				case SDK_ENC_H264_MODE_VBR: {
					HI_U32 TemplateITHRDI[12] = {5,5,5,10,10,10,255,255,255,255,255,255};
					HI_U32 TemplateITHRDP[12] = {5,5,5,255,255,255,255,255,255,255,255,255};
					VENC_RC_PARAM_S tmpRcParam;

					SOC_CHECK(HI_MPI_VENC_GetRcPara(venc_ch, &tmpRcParam));
					memcpy(tmpRcParam.u32ThrdI, TemplateITHRDI, sizeof(TemplateITHRDI));
					memcpy(tmpRcParam.u32ThrdP, TemplateITHRDP, sizeof(TemplateITHRDP));
					tmpRcParam.u32QpDelta                        = 3;
					tmpRcParam.stParamH264VBR.s32DeltaQP         = 10;
					tmpRcParam.stParamH264VBR.s32ChangePos       = 50;
					tmpRcParam.stParamH264VBR.u32MinIprop        = 5;
					tmpRcParam.stParamH264VBR.u32MaxIprop        = 50;
					// tmpRcParam.stParamH264VBR.bLostFrmOpen;
					// tmpRcParam.stParamH264VBR.u32LostFrmBpsThr;
					// tmpRcParam.stParamH264VBR.enSuperFrmMode = SUPERFRM_REENCODE;
					// tmpRcParam.stParamH264VBR.u32SuperIFrmBitsThr;
					// tmpRcParam.stParamH264VBR.u32SuperPFrmBitsThr;
					tmpRcParam.stParamH264VBR.u32MinIQP          = 25;
					SOC_CHECK(HI_MPI_VENC_SetRcPara(venc_ch, &tmpRcParam));
				}
				break;
				...
				case SDK_ENC_H264_MODE_ABR: {
					...
					memcpy(tmpRcParam.u32ThrdP, TemplateITHRDP, sizeof(TemplateITHRDP));
					tmpRcParam.u32QpDelta                        = 3;
					tmpRcParam.stParamH264CbrV2.u32MinIprop      = 1;
					tmpRcParam.stParamH264CbrV2.u32MaxIprop      = 10;
					tmpRcParam.stParamH264CbrV2.u32MaxQp         = 51;
					tmpRcParam.stParamH264CbrV2.u32MaxStartQp    = 51;
					tmpRcParam.stParamH264CbrV2.u32MinQp         = 10;
					// tmpRcParam.stParamH264CbrV2.bLostFrmOpen;
					// tmpRcParam.stParamH264CbrV2.u32LostFrmBpsThr;
					// tmpRcParam.stParamH264CbrV2.enSuperFrmMode;
					// tmpRcParam.stParamH264CbrV2.u32SuperIFrmBitsThr;
					// tmpRcParam.stParamH264CbrV2.u32SuperPFrmBitsThr;
					tmpRcParam.stParamH264CbrV2.s32IPQPDelta     = 6;
					tmpRcParam.stParamH264CbrV2.s32QualityLevel  = 3;
					tmpRcParam.stParamH264CbrV2.s32MaxReEncodeTimes = 2;
					tmpRcParam.stParamH264CbrV2.u32MinIQp        = 10;
					SOC_CHECK(HI_MPI_VENC_SetRcPara(venc_ch, &tmpRcParam));
				}
				break;
			}
...
static int enc_snapshot(int vin, SDK_ENC_SNAPSHOT_QUALITY_t quality, size_t width, size_t height, FILE* stream)
{
	if(vin < HI_VENC_CH_BACKLOG_REF){
		...
		int const venc_grp_jpeg = SNAPSHOT_JPEG_GRP;
		int const venc_chn_jpeg = SNAPSHOT_JPEG_CHN;
		int const vpss_group = vin;
		int const vpss_ch = VPSS_BYPASS_CHN; 
		
		VPSS_GRP_ATTR_S stGrpAttr;
		if(HI_MPI_VPSS_GetGrpAttr(vpss_group, &stGrpAttr)!= HI_SUCCESS)
		{
			return SDK_FAILURE;
		}
		...
		int const width_max = stGrpAttr.u32MaxW;
		int const height_min = HI_VENC_JPEG_MIN_HEIGHT;
		int const height_max = stGrpAttr.u32MaxH;
		...
		if(SDK_ENC_SNAPSHOT_DEFAULT_SIZE == width || SDK_ENC_SNAPSHOT_DEFAULT_SIZE == height){
			width = stGrpAttr.u32MaxW / 2;
			height = stGrpAttr.u32MaxW / 2;
		}else{
