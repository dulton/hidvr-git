...
typedef struct HTTP_HEADER
{
	...
	struct HTTP_HEADER_TAG* tags;

	// interfaces
	int (*add_tag_text)(struct HTTP_HEADER* const header, const char* tag_name, const char* tag_val);
	int (*add_tag_int)(struct HTTP_HEADER* const header, const char* tag_name, int const tag_val);

	int (*add_server)(struct HTTP_HEADER* const header); // FIXME: add default server name
	int (*add_date)(struct HTTP_HEADER* const header); // FIXME:
	
	int (*del_tag)(struct HTTP_HEADER* const header, const char* tag_name);
	int (*to_text)(struct HTTP_HEADER* const header, char* text, ssize_t text_size);
	int (*dump)(struct HTTP_HEADER* const header);
