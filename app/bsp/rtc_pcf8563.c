...
void pcf8563_i2c_read(unsigned char Addr, unsigned char Buf[], int Size)
{
    gpio_i2c_rd_arr(0, PCF8563_ADDR, &Addr, sizeof(Addr), Buf, Size);
}

void pcf8563_i2c_write(unsigned char Addr, unsigned char Buf[], int Size)
{
    gpio_i2c_wr_arr(0, PCF8563_ADDR, &Addr, sizeof(Addr), Buf, Size);
}
...
static DEFINE_IOCTL(rtc_ioctl, file, cmd, arg)
{
	int ret = -EINVAL;
