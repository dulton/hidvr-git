...
#define HI_VENC_OVERLAY_CANVAS_STOCK_REF (HI_VENC_OVERLAY_BACKLOG_REF * HI_VENC_OVERLAY_BACKLOG_REF * HI_VENC_OVERLAY_BACKLOG_REF)
...
typedef struct SDK_ENC_ATTR
{
	...
	SDK_ENC_VIDEO_OVERLAY_ATTR_SET_t video_overlay_set[HI_VENC_CH_BACKLOG_REF];
...
//	SOC_TRACE("Querying venc ch %d", venc_id);
	if(HI_SUCCESS == HI_MPI_VENC_Query(venc_ch, &venc_chn_stat)){
		if(venc_chn_stat.u32CurPacks > 0){
			...
			if(buf_id >= 0){
				...
				in_data_size = sizeof(SDK_ENC_BUF_ATTR_t) + attr.data_sz;
				//SOC_TRACE("get data size(%d) from ref(%d) venc(%d) to buf(%d)", attr.h264.ref_counter, in_data_size, venc_id, buf_id);
				if(1){//if(0 == attr.h264.ref_counter || 2 == attr.h264.ref_counter){
					if(1){//if(in_data_size < stream_attr->frame_limit){
						...
						if(0 == do_buffer_request(buf_id, in_data_size, attr.h264.keyframe)){	
							...
							do_buffer_attach(buf_id, &attr, sizeof(attr));
							// buffer in the payload
							for(i = 0; i < venc_stream.u32PackCount; ++i){
								do_buffer_attach(buf_id, venc_stream.pstPack[i].pu8Addr[0], venc_stream.pstPack[i].u32Len[0]);
								if(venc_stream.pstPack[i].u32Len[1] > 0){
									do_buffer_attach(buf_id, venc_stream.pstPack[i].pu8Addr[1], venc_stream.pstPack[i].u32Len[1]);
								}
							}
						...
						// request a new idr
						SOC_TRACE("drop a frame size = %d", in_data_size);
...
static int enc_snapshot(int vin, SDK_ENC_SNAPSHOT_QUALITY_t quality, ssize_t width, ssize_t height, FILE* stream)
{
	if(vin < HI_VENC_CH_BACKLOG_REF){
		...
		switch(quality){
			case SDK_ENC_SNAPSHOT_QUALITY_HIGHEST:
			case SDK_ENC_SNAPSHOT_QUALITY_HIGH:
			case SDK_ENC_SNAPSHOT_QUALITY_MEDIUM: 
			case SDK_ENC_SNAPSHOT_QUALITY_LOW:
...
//		SOC_TRACE("Snapshot @ %d (%d, %d)!", vin, venc_attr_jpeg.u32PicWidth, venc_attr_jpeg.u32PicHeight);
...
static SDK_ENC_VIDEO_OVERLAY_ATTR_t* enc_lookup_overlay_byname(int vin, int stream, const char* name)
{
	...
	if(vin < HI_VENC_CH_BACKLOG_REF
		&& 0 == stream){
		SDK_ENC_VIDEO_OVERLAY_ATTR_SET_t* const overlay_set = &_sdk_enc.attr.video_overlay_set[vin];
		// check name override
	...
	if(NULL != canvas && vin < HI_VENC_CH_BACKLOG_REF && 0 == stream){
		...
		for(i = 0; i < HI_VENC_OVERLAY_BACKLOG_REF; ++i){
			...
			if(!overlay->canvas){
				...
				SOC_TRACE("Create overlay @ %d name %s", overlay->region_handle, overlay->name);
...
static enc_show_overlay(int vin, int stream, const char* overlay_name, bool show)
{
	if(vin < HI_VENC_CH_BACKLOG_REF && 0 == stream){
		SDK_ENC_VIDEO_OVERLAY_ATTR_t* const overlay = enc_lookup_overlay_byname(vin, stream, overlay_name);
		if(NULL != overlay){
...
static enc_update_overlay(int vin, int stream, const char* overlay_name)
{
	if(vin < HI_VENC_CH_BACKLOG_REF && 0 == stream){
		SDK_ENC_VIDEO_OVERLAY_ATTR_t* const overlay = enc_lookup_overlay_byname(vin, stream, overlay_name);
		if(NULL != overlay){
...
static enc_update_overlay_byname(const char* stream_name, const char* overlay_name)
{
	int vin, stream;
	if(SDK_SUCCESS == sdk_enc->lookup_stream_byname(stream_name, &vin, &stream)){
		return sdk_enc->update_overlay(vin, stream, overlay_name);
	}
	return SDK_FAILURE;
}
...
int SDK_init_enc()
{
	...
	if(NULL == sdk_enc){
		...
		for(i = 0; i < HI_VENC_CH_BACKLOG_REF; ++i){
			SDK_ENC_VIDEO_OVERLAY_ATTR_SET_t* const overlay_set = &_sdk_enc.attr.video_overlay_set[i];
