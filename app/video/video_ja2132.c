...
#define RefSizW16 (RefSizW/2)
#define RefSizH16 ((8 * (RefSizH/2) + 7 * 2)*2)
...
static void video_DisplayD1(int nVoDev, int nChn, unsigned char bDisplay)
{
	...
	if(bDisplay) {
		...
		if(s_nLstViChn > (MAX_CAM_CH/16)*16) {
			int ViChn = s_nLstViChn - (MAX_CAM_CH/16)*16+(MAX_CAM_CH/16)*4;

			DVR_ASSERT(HI_MPI_VI_GetChnAttr(ViChn, &tmpChnAttr));
		...
		s_nLstViChn = nChn;
		if(s_nLstViChn >= (MAX_CAM_CH/16)*16) {
			ViChn = s_nLstViChn - (MAX_CAM_CH/16)*16+(MAX_CAM_CH/16)*4;
			...
			if(fd > 0) {
				...
				stPreview.chn = s_nLstViChn; // switch to this channel @ ad
				ioctl(fd, VIDEO_DRV_SET_LIVE_CHN, &stPreview);
				close(fd);
				fd = -1;
			}
			ViChn = (MAX_CAM_CH/16)*4+(MAX_CAM_CH%16)+s_nLstViChn/16*4;
		}
...
#if   defined(SDK_PLATFORM_HI3521) && (MAX_CAM_CH == 24)
        0xFF000000, 0xFF00, 0xFF, 0xFF0000,
#elif defined(SDK_PLATFORM_HI3521) && (MAX_CAM_CH == 32)
...
VI_DEV_ATTR_S DEV_ATTR_BT656D1_4MUXI =
{
    /*接口模式*/
    VI_MODE_BT656,
    /*1、2、4路工作模式*/
...
VI_DEV_ATTR_S DEV_ATTR_BT656D1_4MUXP =
{
    /*接口模式*/
    VI_MODE_BT656,
    /*1、2、4路工作模式*/
...
VI_DEV_ATTR_S DEV_ATTR_BT656D1_1MUXI =
{
    /*接口模式*/
    VI_MODE_BT656,
    /*1、2、4路工作模式*/
...
void MUXD1_Mode(int ViChn, int * nVpss);
void MUX16_Mode(int ViChn, int * nVpss);

int  nViDevNum = VIU_MAX_DEV_NUM;

static void video_InputInit()
{
    int ViDev, ViChn, Vpss, ii;
    ...
    for(ViDev = 0; ViDev < nViDevNum; ViDev +=2) {
        ...
        {  //Enable Vi Device;
            ...
            if(ViDev < MAX_CAM_CH/16+(MAX_CAM_CH%16)/4) {
                memcpy(&tmpViDevAttr, &DEV_ATTR_BT656D1_4MUXI, sizeof(tmpViDevAttr));
                VI_MUX_SetMask(ViDev, &tmpViDevAttr);
            }
            else {
                memcpy(&tmpViDevAttr, &DEV_ATTR_BT656D1_1MUXI, sizeof(tmpViDevAttr));
                VI_MUX_SetMask(ViDev, &tmpViDevAttr);
            }
    ...
    for(ii = 0, Vpss = 0, ViChn = 0; ii < MAX_CAM_CH; ViChn ++) {
        if(ii < (MAX_CAM_CH/16)*16) {
            MUX16_Mode(ViChn, &Vpss);
            ii += 4;
        }
        else {
            MUXD1_Mode(ViChn, &Vpss);
            ii ++;
        }

    }
    for(ii = 0; ii < (MAX_CAM_CH/16); ii ++, ViChn += 4) {
        {  //Enable VIU Memory Space;
            ...
            DVR_ASSERT(HI_MPI_SYS_SetMemConf(&stMppChnVI, MMZ_ZONE_NAME0));
        }
    ...
    for(ii = 0; ii < MAX_CAM_CH; ii ++) {
        VPSS_ViGroupSettingInit(ii + MAX_CAM_CH, 1440, VRES_SIZE_H_PL, FALSE, NULL, FALSE);
    }
...
static void video_InputDestroy()
{
    ...
    for(ViChn = 0; ViChn < MAX_CAM_CH; ViChn ++) {
        HI_MPI_Vpss_UnBindVi(ViChn, ViChn);
...
#if  (defined(_HI3515A_EXT) || defined(_HI3520D_EXT)) && !defined(_HI3520D_RAM_LITE)
...
void MUXD1_Mode(int ViChn, int * nVpss)
{
    ...
    {  //Enable VIU Memory Space;
        ...
        DVR_ASSERT(HI_MPI_SYS_SetMemConf(&stMppChnVI, ViChn%2 ?MMZ_ZONE_NAME0 :MMZ_ZONE_NAME1));
    }
    {  //Enable VIU Channel Attribute;
        VI_CHN_ATTR_S tmpChnAttr;
    ...
    {  //Enable VPSS Binding To Vi
        VPSS_ViGroupSettingInit(Vpss, RefSizW, RefSizH, FALSE, NULL, FALSE);
        {
            ...
            DVR_ASSERT(HI_MPI_VPSS_SetPreScale(Vpss, &tmpPreScale));
        }

        HI_MPI_Vpss_BindVi(Vpss, ViChn);
    }
    Vpss  ++;
    *nVpss = Vpss;
}
...
void MUX16_Mode(int ViChn, int * nVpss)
{
    ...
    {  //Enable VIU Channel Attribute;
        ...
        tmpChnAttr.stCapRect.s32X       = 0;
        tmpChnAttr.stCapRect.s32Y       = 0;
        tmpChnAttr.stCapRect.u32Width   = 720;
        tmpChnAttr.stCapRect.u32Height  = RefSizH;
        tmpChnAttr.enCapSel             = VI_CAPSEL_BOTH;
        tmpChnAttr.stDestSize.u32Width  = 720;
        tmpChnAttr.stDestSize.u32Height = RefSizH;
        tmpChnAttr.enPixFormat          = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
        tmpChnAttr.bMirror              = HI_FALSE;
        tmpChnAttr.bFlip                = HI_FALSE;
    ...
    for(ii = 0; ii < 4; ii ++) {  //Enable VPSS Binding To Vi
        VPSS_CROP_INFO_S tmpCrop;
        tmpCrop.bEnable              = 1;
        tmpCrop.enCropCoordinate     = VPSS_CROP_ABS_COOR;
        tmpCrop.stCropRect.s32X      = (RefSizW/2+8)*(ii%2);
        tmpCrop.stCropRect.s32Y      = 0;
        tmpCrop.stCropRect.u32Width  = RefSizW/2;
        tmpCrop.stCropRect.u32Height = RefSizH;
        tmpCrop.enCapSel             = ii/2 ?VPSS_CAPSEL_TOP :VPSS_CAPSEL_BOTTOM;
        VPSS_ViGroupSettingInit(Vpss, 720, RefSizH, FALSE, &tmpCrop, FALSE);
